...
async function main() {
  ...
  const activeCount = [hasStdio, hasSse, hasStreamableHttp, hasConfig].filter(
    Boolean,
  ).length
  ...
  if (activeCount === 0) {
    logger.error(
      'Error: You must specify one of --stdio, --sse, --streamableHttp, or --config',
    )
    process.exit(1)
  } else if (activeCount > 1) {
    logger.error(
      'Error: Specify only one of --stdio, --sse, --streamableHttp, or --config, not multiple',
    )
    process.exit(1)
  }
  ...
  try {
    if (hasStdio) {
      ...
      if (argv.outputTransport === 'sse') {
        ...
        let sessionTimeout: null | number = null
