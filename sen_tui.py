...
# Thêm đường dẫn hiện tại để import các module nội bộ
sys.path.insert(0, os.getcwd())

# Import SEN Brain và user manager
try:
...
try:
    ...
    print("⚠️ Không có user_manager, bỏ qua đăng nhập.")
...
def process_intent(text):
    ...
    elif text_lower in ("/exit", "/thoat"):
        return "EXIT"
    elif text_lower == "/help":
        return ("🔧 Lệnh:\n  /exit, /help, /status\n💬 Nghiệp vụ:\n  dự báo [ngân sách] [số bài] [doanh số dự kiến]\n  cảm xúc [nội dung]\n  tối ưu [ngân sách] [số bài]\n⚙️ Lệnh mới:\n  !optimize <code>\n  !sandbox <code>\n  !check_affiliate <message>")
    elif text_lower == "/status":
        return "🧠 SEN Brain đang hoạt động."
    else:
        return None
...
class LoginDialog(tk.Toplevel):
    ...
    def login(self):
        ...
        if AUTH_AVAILABLE:
            ...
            # Fallback: cho phép nếu user/pwd là admin/admin
            if user == "admin" and pwd == "admin":
                self.result = type('User', (), {'id':1, 'username':'admin', 'role':'admin'})
                self.destroy()
                return
...
class ChatWindow:
    def __init__(self, root, user):
        ...
        self.append_message("Hệ thống", f"Chào {user.username}, bạn có vai trò {user.role}. {AI_IDENTITY} sẵn sàng.", system=True)
    ...
    def append_message(self, sender, text, system=False):
        ...
        if system:
            ...
            self.chat_area.insert(tk.END, f"\n👤 {sender}: {text}\n", "user" if sender == "Anh Đức" else "sen")
    ...
    def send_message(self, event=None):
        ...
        self.append_message("Anh Đức", user_input)

        # Xử lý lệnh đặc biệt mới (bắt đầu bằng !)
        if user_input.startswith("!"):
            response = self.handle_command(user_input)
            self.append_message(AI_IDENTITY, response)
            return

        # Xử lý lệnh cũ và Ollama
        ...
        if result == "EXIT":
            ...
            self.chat_area.config(state=tk.NORMAL)
            # Tìm và xóa dòng cuối (dòng đang suy nghĩ)
    ...
    def handle_command(self, cmd):
        """Xử lý các lệnh mở rộng !optimize, !sandbox, !check_affiliate"""
        if not BRAIN_READY:
            return "⚠️ SEN Brain chưa sẵn sàng."
        if cmd.startswith("!optimize "):
            ...
            # Giả sử user_id lấy từ self.user.id hoặc test
            ...
            return "❗ Lệnh không hợp lệ. Các lệnh: !optimize <code>, !sandbox <code>, !check_affiliate <message>"
